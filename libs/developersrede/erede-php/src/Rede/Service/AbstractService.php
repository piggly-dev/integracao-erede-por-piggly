...
use Piggly\WooERedeGateway\Vendor\Psr\Log\LoggerInterface;
use Piggly\WooERedeGateway\Vendor\Rede\eRede;
...
abstract class AbstractService
{
    ...
    {
        $userAgent = \sprintf('User-Agent: %s', \sprintf(eRede::USER_AGENT, \phpversion(), $this->store->getFiliation(), \php_uname('s'), \php_uname('r'), \php_uname('m')));
        ...
        if (\is_array($curlVersion)) {
            $userAgent .= \sprintf(' curl/%s %s', isset($curlVersion['version']) ? $curlVersion['version'] : '', isset($curlVersion['ssl_version']) ? $curlVersion['ssl_version'] : '');
        }
        $headers = [\str_replace('  ', ' ', $userAgent), 'Accept: application/json', 'Transaction-Response: brand-return-opened'];
        $this->curl = \curl_init($this->store->getEnvironment()->getEndpoint($this->getService()));
        \curl_setopt($this->curl, \CURLOPT_USERPWD, \sprintf('%s:%s', $this->store->getFiliation(), $this->store->getToken()));
        if (!\defined('CURL_SSLVERSION_TLSv1_2')) {
            throw new RuntimeException(\sprintf('Atenção, sua versão da curl não suporta TLS 1.2 e precisa ser atualizada. Sua versão atual da curl é %s', $curlVersion));
        }
        \curl_setopt($this->curl, \CURLOPT_SSLVERSION, \CURL_SSLVERSION_TLSv1_2);
        \curl_setopt($this->curl, \CURLOPT_SSL_VERIFYPEER, \true);
        switch ($method) {
            case 'GET':
                break;
            case 'POST':
     ...
     * @return mixed
     */
    protected abstract function parseResponse($response, $statusCode);
}
